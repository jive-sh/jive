...
- The team that owns library "Brittle" isn't very disciplined with their coding practices; their changes are always breaking the pipelines of other team's apps and libraries, notable the pipeline of the "Mission Critical" app.
- People keep breaking violating the boundaries that the modules are supposed to be setting, they keep writing code which references code in another area of the codebase. It's difficult to know how much dependency is properly managed by standard package dependency rules and how much is just relative links across the massive codebase
